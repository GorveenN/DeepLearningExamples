...
class pytorch_pruning(object):
    def __init__(self, parameters, pruning_settings=dict(), log_folder=None):
        ...
        self.sum_of_iterations = 0
        self.sum_of_losses = 0
        ...
        for layer, if_prune in enumerate(self.prune_layers):
            ...
            if self.method == 0:
                ...
                if 1:
                    ...
                    criteria_for_layer = criteria_for_layer / (criteria_for_layer.norm().item() + eps)
            elif self.method == 10:
                # diagonal of Hessian
                ...
                #print(self.parameters[layer].double())
                #print("GRAD")
                #print(self.parameters[layer].grad)
                criteria_for_layer = (self.parameters[layer].double()*self.parameters[layer].grad.double()).data.pow(2).view(nunits, -1).sum(dim=1)
    ...
    def compute_saliency(self):
        ...
        for layer, if_prune in enumerate(self.prune_layers):
            ...
            if self.iterations_done > 0:
                ...
                exit()
            print(len(self.parameters[layer]))
            for unit in range(len(self.parameters[layer])):
        ...
        print("Pruned neurons: {} prune per iteration: {} group size: {}", self.pruned_neurons, self.prune_per_iteration, self.group_size)
        prune_neurons_now = (self.pruned_neurons + self.prune_per_iteration)//self.group_size - 1
        if self.prune_neurons_max != -1:
            print("In if, all_criteria: {}", len(all_criteria))
        ...
        print("prune_neurons now ", prune_neurons_now)
        print(np.sort(all_criteria))
        threshold_now = np.sort(all_criteria)[prune_neurons_now]
        print("threshold ", threshold_now)
    ...
    def run_full_oracle(self, model, data, gloc, glabel, criterion, mean, std, encoder, initial_loss):
        '''
        ...
        if self.method == 40:
            ...
            for layer_index, layer_parameters in enumerate(self.parameters):
                ...
                print("starting layer: ", layer_index)
                if len(self.oracle_dict["loss_list"]) < layer_index + 1:
                    self.oracle_dict["loss_list"].append(list())
    ...
    def do_step(self, loss=None, optimizer=None, neurons_left=0, training_acc=0.0, file=None):
        '''
        ...
        self.add_criteria()
        self.sum_of_iterations += 1
        if loss is not None:
            self.sum_of_losses += loss
        ...
        if niter % self.frequency == 0 and niter != 0:
            ...
            if self.res_pruning == 1:
                ...
                if file is not None:
                    f = open(file, "a")
                    f.write(str(self.pruning_iterations_done) + " ")
                    f.write(str(self.sum_of_losses/self.sum_of_iterations) + "\n")
                    f.close()
                    self.sum_of_losses = 0
                    self.sum_of_iterations = 0
                    ...
                    if pruning_method == 6:
                        ...
                        def just_hook(self, grad_input, grad_output):
                            if len(grad_output[0].shape) == 4:
                                ...
                                self.weight.full_grad_iclr2017 = (grad_output[0] * self.output).abs().mean(0).mean(0)
                                #print(self.weight.full_grad_iclr2017.shape)
